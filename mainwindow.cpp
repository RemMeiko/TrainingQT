    ...
    ui(new Ui::MainWindow)
{
    ...
    /*窗口标题*/
    this->setWindowTitle("欢迎来到智能家居管理系统");
...
//    ui->imageSwitch1->setButtonStyle(ImageSwitch::ButtonStyle_2);
    ...
    // connect(timer,SIGNAL(timeout()),this,SLOT(on_btn_show2_start_collect_clicked()));
...
//    ui->customPlot->replot();
    ...
    // timer2->start();
    timer2->setInterval (2500) ;//设置定时周期，单位：毫秒
    connect(timer2,SIGNAL(timeout()),this,SLOT(collect()));
    timer2->stop();
...
void MainWindow::collect()
{
    ...
    int rand_temp = qrand() % 2 + 25;   //23~32的随机数 25-26
    int rand_humanity = qrand() % 3 + 45;  //55~90的随机数  qrand()%(b-a+1)+a a~b的随机数   45 47
...
//    query.exec("insert into dataforht values (12,79,43)");
//    query.exec("insert into dataforht values (13,34,56)");
//    query.prepare("insert into dataforht values (?,?,?)");
//    query.addBindValue(14);
//    query.addBindValue(rand_temp);
//    query.addBindValue(rand_humanity);
//    query.exec();
...
//    query.exec();
    ...
    query.addBindValue(rand_humanity);
    query.exec();
    model->setQuery("select * from dataforht"); //再次查询整张表
    ui->datashow1_tableview->setModel(model); //再次进行显示
    qDebug() <<"ok!" << endl;
}
...
//开始收集按钮的槽函数
void MainWindow::on_btn_show2_start_collect_clicked()
{
    timer2->start();//定时器2开始，执行定时器2的collect()函数，每3秒获取温湿度数据
    //qDebug() << "我已点击" << endl;
}


//停止收集按钮的槽函数（利用定时器结束进行停止）
void MainWindow::on_btn_show2_stop_collect_clicked()
{
    timer2->stop();
}
